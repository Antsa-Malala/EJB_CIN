    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
           Properties p = new Properties();
            ...
            ArrayList<TanyDTO> list=(ArrayList<TanyDTO>)ta.liste_tany("1234567890");
            for(int i=0;i<list.size();i++)
            {
                out.println(list.get(i).getnumCadastre());
                out.println(list.get(i).getadresseTany());
                out.println(list.get(i).getsuperficie());
                out.println(list.get(i).gettypeTany());
                out.println(list.get(i).getproprio().getnom()+list.get(i).getproprio().getprenom());
            }
        }
        catch(Exception e)
        {
